...
ggplot(data_movie1_2,aes(x=date,y=average,fill=date)) +
  geom_bar(stat="identity") +
  xlab("날짜") + ylab("평균 평점") +
  ...
  theme(plot.title = element_text(color="black",size=20,face="bold.italic",hjust=0.5),
        ...
        axis.text.x = element_text(angle=45)) +
  geom_text(aes(y=average,label=ylabel),color="black")
ggplot(data_movie1_3,aes(x=date,y=average,fill=date)) +
  geom_bar(stat="identity") +
  xlab("날짜") + ylab("평균 평점") +
  ...
  theme(plot.title = element_text(color="black",size=20,face="bold.italic",hjust=0.5),
        ...
        axis.text.x = element_text(angle=45)) +
  geom_text(aes(y=average,label=ylabel),color="black")
ggplot(data_movie1_4,aes(x=date,y=average,fill=date)) +
  geom_bar(stat="identity") +
  xlab("날짜") + ylab("평균 평점") +
...
ggplot(data_movie2,aes(x=hour,y=average)) +
  ...
  geom_text(aes(y=average,label=ylabel),color="darksalmon")
```
...
ggplot(data_movie3,aes(x=hour,y=average,colour=date)) +
  ...
  theme(plot.title = element_text(color="black",size=20,face="bold.italic",hjust=0.5),
        axis.title.x = element_text(color="red",size=15,face="bold"),
        axis.title.y = element_text(color="blue",size=15,face="bold"),
        axis.text.x = element_text(face="bold",color="brown",size=10,angle=45),
        axis.text.y = element_text(face="bold",color="brown",size=10,angle=45))
```
...
data_movie3_4 <- data_movie3 %>%
  filter(date %in% c('2019.06.26','2019.06.27','2019.06.28','2019.06.29','2019.06.30',
                     '2019.07.01','2019.07.02','2019.07.03','2019.07.04','2019.07.05'))
require(ggplot2)
ggplot(data_movie3_1,aes(x=hour,y=average,colour=date)) +
  geom_line(size=1) +
  xlab("시간대") + ylab("평균 평점") +
  ...
  theme(plot.title = element_text(color="black",size=20,face="bold.italic",hjust=0.5),
        ...
        axis.text.x = element_text(face="bold",color="brown",size=10,angle=45),
        axis.text.y = element_text(face="bold",color="brown",size=10,angle=45))
ggplot(data_movie3_2,aes(x=hour,y=average,colour=date)) +
  geom_line(size=1) +
  xlab("시간대") + ylab("평균 평점") +
  ...
  theme(plot.title = element_text(color="black",size=20,face="bold.italic",hjust=0.5),
        ...
        axis.text.x = element_text(face="bold",color="brown",size=10,angle=45),
        axis.text.y = element_text(face="bold",color="brown",size=10,angle=45))
ggplot(data_movie3_3,aes(x=hour,y=average,colour=date)) +
  geom_line(size=1) +
  xlab("시간대") + ylab("평균 평점") +
  ...
  theme(plot.title = element_text(color="black",size=20,face="bold.italic",hjust=0.5),
        ...
        axis.text.x = element_text(face="bold",color="brown",size=10,angle=45),
        axis.text.y = element_text(face="bold",color="brown",size=10,angle=45))
ggplot(data_movie3_4,aes(x=hour,y=average,colour=date)) +
  geom_line(size=1) +
  xlab("시간대") + ylab("평균 평점") +
  ...
  theme(plot.title = element_text(color="black",size=20,face="bold.italic",hjust=0.5),
        ...
        axis.text.y = element_text(face="bold",color="brown",size=10,angle=45))
```
<br> 마지막으로 평점 별 빈도수는 어떻게 되는지 확인해보겠다. <br>
```{r}
# 평점 별 빈도수 데이터
...
data_movie4
```
이제 생성된 데이터 data_movie4를 이용해 평점 별 빈도수 관련 barplot를 그려보도록 한다. <br>
```{r}
# barplot of data_movie4
...
```
<br> 위의 그래프를 통해서 봤을 때 만점(10점)을 준 네티즌의 수가 압도적으로 많음을 알 수 있다. <br>

그렇다면, 과연 일자별, 시간대별로 네티즌 평균평점이 차이가 있다고 할 수 있는지 통계분석을 실시해보도록 하겠다. 여기에서는 검증하고자 하는 요소가 일자별(date), 시간대별(hour) 이렇게 2가지이므로 일원배치법(one-way ANOVA test)과 이원배치법(two-way ANOVA test)을 적용한다. <br>
...
추가적으로 모수적 검정과 비모수적 검정에 대한 간략한 개괄은 아래의 사이트에 나와있다. <br>
site : https://keydifferences.com/difference-between-parametric-and-nonparametric-test.html <br>
...
실험계획법(experimental design), 혼합모형(mixed model)에서의 random effect, 그리고 fixed effect와 관련한 내용은 아래의 사이트와 pdf 파일을 참조하기 바란다. <br>
site : http://conjugateprior.org/2013/01/formulae-in-r-anova/ <br>
pdf : http://www.statpower.net/Content/311/Lecture%20Notes/RandomEffects.pdf <br>
...
```
<br> 결과를 살펴보면, 어느정도 걸러질 문자나 어구는 대부분 사라진 것으로 보인다. 이 결과를 바탕으로 wordcount table을 생성하도록 하고, 이를 통해 결과를 정리하면 다음과 같다. 그전에 사용빈도수가 30 이하인 단어는 wordcloud 생성 시 나타나지 않을 수 있고, 이유없이 다른 사람을 비하하는 비속어나 정치적인 내용 등이 담겨있는 중요하지 않은 어구이므로 제외시키도록 한다. <br> 
```{r}
...
```
<br> wordcloud2를 이용하면 좀 더 세련된 그림으로 표현이 가능하다. <br>
```{r}
require(wordcloud2)
wordcloud2(wordcount[wordcount>30],size=2,col="random-light",rotateRatio=0.5,
           backgroundColor="black",shape="star")
```
<br> 결과를 보면 대체적으로 생각, 여운, 현실, 소름 등의 느낌과 관련한 단어와 역대, 관람객, 최고 등의 평가와 관련한 단어가 많이 추출됨을 확인할 수 있다. 하지만 결말을 통해 뭔가 충격적이고 씁쓸하다고 생각하는 네티즌들도 많아보인다. 아무래도 국제적으로 인정받는 상을 수상한 영화이니 만큼 대단하다는 평가가 절대적이라고 보여진다.    
